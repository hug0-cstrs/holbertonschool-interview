...
{
	int *buff; /* buffer to store data */

	if (size < 2) /* if size is less than 2 */
		return;

	buff = malloc(sizeof(int) * size); /* allocate memory for buffer */
	if (buff == NULL)
		return;

	copy(array, buff, size); /* copy data to buffer */

	split(array, buff, 0, size - 1, size); /* split data */
